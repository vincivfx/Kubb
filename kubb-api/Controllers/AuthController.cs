...
public class AuthController(DatabaseContext context, TurnstileService turnstileService, EmailTask emailTask, IConfiguration configuration) : BaseController
{
    ...
    public ActionResult RecoverPassword([FromBody] RecoverPasswordRequest request)
    {
        ...
        if (user == null || (user.Status & UserStatus.Active) != UserStatus.Active || user.LastRecoverRequiredTime != null && user.LastRecoverRequiredTime.Value.AddHours(2) > DateTime.UtcNow)
        {
            return new UnauthorizedResult();
        }
    ...
    public ActionResult CompleteRecoverPassword([FromBody] CompleteRecoverPasswordRequest request)
    {
        ...
        if (user == null || user.LastRecoverRequiredTime == null || user.LastRecoverRequiredTime.Value.AddHours(2) < DateTime.UtcNow || !user.VerifyRecoveryToken(request.Token))
        {
            return Unauthorized();
        }
